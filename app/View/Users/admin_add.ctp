	    ...
	    <fieldset>
	    	...
	    	<?php
				...
				echo $this->Form->input('org_group_id', array(
					'div' => array('class' => 'third'),
					'description' => __('Changing this will update the below that have the same value this was set to, and are visible, to the new value'),
 				));
...
foreach ($clients as $i => $client)
{
	...
	$td[$i]['Client.org_group_id'] = $this->Form->input('ClientsUser.'.$client['Client']['client_id'].'.org_group_id', array(
		'div' => false,
		'label' => false,
		'type' => 'select',
		'options' => $orgGroups,
...
$(document).ready(function ()
{
	...
	////// when the org group is set, set the org group for the clients as well
	// this should only happen on this page, not the edit page
	var previousVal = 0;
	$('#UserOrgGroupId')
	...
	.on('change', function() {
		// find all of the org groups below, and change the ones that are set to this previous, to this new value
		var parentVal = $(this).val();
		$('td select.org_group_id_child').each(function(){
			if($(this).val() == previousVal && $(this).parent().is(':visible'))
			{
				$(this).val(parentVal);
