...
$th = array(
	...
	'LoginHistory.remote_user' => array('content' => __('Remote User'), 'options' => array('sort' => 'LoginHistory.remote_user')),
...
foreach ($loginHistories as $i => $login_history)
{
	...
	if($login_history['LoginHistory']['user_id'] > 0)
	{
		...
		$user = $this->Html->link($tmp['User']['name'], array('controller' => 'users', 'action' => 'view', $tmp['User']['id']));
	}
	$td[$i] = array(
		$email,
		$user,
		$login_history['LoginHistory']['remote_user'],
